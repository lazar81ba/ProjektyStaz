...
public class ExportFromDAO implements DAOOperation {
	...
	private void parseAndClearList() throws XMLParserException {
		if (parser != null)
			parser.parsePhoneList(list);
		else
			throw new XMLParserException("No parser is initialized");
		list.clear();
	}
	...
	public void executeOperation(String filename) throws DatabaseException, XMLParserException {
		try {
			initialize();
			wrapper.connect();
